...
class SpecialtyAdapter(private val context: Context?, private val clickListener: ItemClickListener):RecyclerView.Adapter<SpecialtyAdapter.SpecialtyViewHolder>() {

    var list = listOf<UserResponse>()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SpecialtyViewHolder {
        val view = LayoutInflater.from(context).inflate(R.layout.recycler_spec_item,parent,false)
    ...
    fun setSpecialization(specialty: List<UserResponse>) {
        this.list = specialty
    ...
    class SpecialtyViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: UserResponse){
            tvSpec.text = item.specialty?.get(0)?.name
        }
    }
}